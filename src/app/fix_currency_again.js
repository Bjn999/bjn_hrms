...
function processDir(dir) {
    ...
    for (const f of files) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            // Match `?.?` or `د.ك` directly (as text encoded strangely)
            if (content.includes('د.ك')) {
                content = content.replace(/د\.ك/g, "{t('currency')}");
                changed = true;
            }

            // Also search for things like `>رس<` or `رس` 
            if (content.includes('رس')) {
                // If it's a raw string in jsx like: <span>رس</span>
                content = content.replace(/>\s*رس\s*</g, ">{t('currency')}<");
                content = content.replace(/\sرس\s/g, " {t('currency')} ");
                changed = true;
            }
