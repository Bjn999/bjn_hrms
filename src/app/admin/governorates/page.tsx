...
export default function GovernoratesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch('http://localhost:8000/api/admin/governorates', {
  ...
  const fetchCountries = async () => {
    try {
      ...
      const result = await res.json();
      if (result.status) {
        setCountries(result.data.filter((c: any) => c.active == 1));
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const url = modalType === 'add' ? 'http://localhost:8000/api/admin/governorates' : `http://localhost:8000/api/admin/governorates/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`http://localhost:8000/api/admin/governorates/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
      });
      const result = await res.json();
