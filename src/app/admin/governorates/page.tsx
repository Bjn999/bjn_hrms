...
export default function GovernoratesPage() {
  ...
  const [editingItem, setEditingItem] = useState<Governorate | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/governorates`, {
  ...
  const fetchCountries = async () => {
    try {
      ...
      const result = await res.json();
      if (result.status) {
        setCountries(result.data.filter((c: Country) => c.active == 1));
      }
    } catch {
      // Ignored
    }
  ...
  const handleOpenModal = (type: string, item: Governorate | null = null) => {
    setModalType(type);
    if (type === 'edit' && item) {
      setEditingId(item.id);
      setEditingItem(item);
      setFormData({ name: item.name, country_id: item.country_id.toString(), active: item.active.toString() });
    } else {
      setEditingId(null);
      setEditingItem(null);
      setFormData({ name: '', country_id: countries.length > 0 ? countries[0].id.toString() : '', active: '1' });
    }
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const url = modalType === 'add' ? `${process.env.NEXT_PUBLIC_API_URL || ''}/governorates` : `${process.env.NEXT_PUBLIC_API_URL || ''}/governorates/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/governorates/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
      });
      const result = await res.json();
