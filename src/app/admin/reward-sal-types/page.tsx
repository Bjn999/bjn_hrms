...
export default function AdditionalSalTypesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch('http://localhost:8000/api/admin/additional-sal-types', {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = modalType === 'add' 
        ? 'http://localhost:8000/api/admin/additional-sal-types' 
        : `http://localhost:8000/api/admin/additional-sal-types/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`http://localhost:8000/api/admin/additional-sal-types/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json'
