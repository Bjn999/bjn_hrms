...
export default function AdditionalSalTypesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/additional-sal-types`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = modalType === 'add' 
        ? `${process.env.NEXT_PUBLIC_API_URL || ''}/additional-sal-types` 
        : `${process.env.NEXT_PUBLIC_API_URL || ''}/additional-sal-types/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    const isConfirmed = await confirm({
      ...
      description: 'هل انت متأكد من حذف هذه الإضافة المالية؟',
      icon: 'danger'
    });
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/additional-sal-types/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json'
    ...
    <>
      <div className="animate-fade-in-up pb-10">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h2 className="text-3xl font-black text-slate-800 tracking-tight">{t('additional_sal_types')}</h2>
          </div>
          <button 
          ...
          <div className="p-6 overflow-x-auto">
            <table className="w-full text-sm text-right">
              <thead className="bg-slate-50 text-slate-700 uppercase border-b border-slate-100">
                <tr>
                  <th className="px-6 py-4">{t('additional_sal_type_name')}</th>
      ...
      {mounted && showModal && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8 animate-zoom-in relative overflow-hidden">
            ...
            <h3 className="text-2xl font-black text-slate-800 mb-6">{modalType === 'add' ? t('add_additional_sal_type') : t('edit_additional_sal_type')}</h3>
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-base font-black text-slate-950 mb-2">{t('additional_sal_type_name')}</label>
