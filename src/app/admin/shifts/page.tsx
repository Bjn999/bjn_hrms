...
export default function ShiftsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch('http://localhost:8000/api/admin/shifts', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = modalType === 'add' 
        ...
        : `http://localhost:8000/api/admin/shifts/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`http://localhost:8000/api/admin/shifts/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json'
                    ...
                    <td className="px-6 py-4 text-slate-900 font-medium whitespace-nowrap">
                      {item.updatedby ? (
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-slate-500">{new Date(item.updated_at).toLocaleString(language === 'ar' ? 'ar-EG' : 'en-US')}</span>
                        </div>
                      ) : (
