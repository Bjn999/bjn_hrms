...
export default function AllowancesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch('http://localhost:8000/api/admin/allowances', {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = modalType === 'add' 
        ? 'http://localhost:8000/api/admin/allowances' 
        : `http://localhost:8000/api/admin/allowances/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`http://localhost:8000/api/admin/allowances/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json'
