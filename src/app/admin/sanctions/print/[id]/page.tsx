...
import { FinanceMonth, Sanction } from '@/types';

interface SanctionPrintItem extends Sanction {
...
const SANCTION_TYPES: Record<number, string> = {
  1: 'جزاء أيام',
  2: 'جزاء مبلغ',
  3: 'جزاء بالبصمة',
};

export default function SanctionsPrintPage() {
  const { t } = useLanguage();
  ...
  const [data, setData] = useState<SanctionPrintItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(`${API}/sanctions/${monthId}`, { headers });
    ...
    <div className="bg-white min-h-screen p-8 text-black" dir="rtl">
      ...
      <div className="flex justify-between items-center border-b-2 border-black pb-4 mb-6">
        ...
        <div className="text-right w-1/3">
          ...
          <h2 className="text-xl font-bold mt-2">كشف جزاءات الموظفين</h2>
        </div>
      ...
      <table className="w-full text-sm border-collapse border border-black text-center">
        <thead>
          <tr className="bg-gray-100">
            ...
            <th className="border border-black p-2 font-bold">نوع الجزاء</th>
            <th className="border border-black p-2 font-bold">أيام</th>
            <th className="border border-black p-2 font-bold">قيمة الجزاء</th>
        ...
        <tbody>
          {data.map((item, index) => (
            <tr key={item.id}>
              ...
              <td className="border border-black p-2">{item.sanctions_type !== undefined ? SANCTION_TYPES[item.sanctions_type] : 'غير محدد'}</td>
          ...
          {data.length === 0 && (
            <tr>
              <td colSpan={7} className="border border-black p-4 text-center">لا توجد بيانات لهذا الشهر</td>
            </tr>
          )}
        </tbody>
      </table>
