...
export default function EmployeeAllowancesDetailPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await res.json();
      if (result.status) {
        showToast('تم حفظ البدل بنجاح', 'success');
        setShowModal(false);
        fetchData();
      } else {
        showToast(result.message, 'error');
  ...
  const handleDelete = async (a: AllowanceItem) => {
    ...
    try {
      ...
      const result = await res.json();
      if (result.status) { showToast('تم حذف البدل بنجاح', 'success'); fetchData(); }
    ...
    <div className="animate-fade-in-up pb-10">
      ...
      <div className="flex justify-between items-start mb-8 flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h2 className="text-2xl font-black text-slate-800">البدلات المتغيرة</h2>
          </div>
          {financeMonth && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-slate-500 font-bold uppercase">عدد البدلات</p>
          ...
          <table className="w-full text-sm text-right">
            <thead className="bg-slate-50 text-slate-700 uppercase border-b border-slate-100">
              <tr>
                ...
                <th className="px-5 py-4">نوع البدل</th>
