...
export default function DiscountSalTypesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/discount-sal-types`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = modalType === 'add' 
        ? `${process.env.NEXT_PUBLIC_API_URL || ''}/discount-sal-types` 
        : `${process.env.NEXT_PUBLIC_API_URL || ''}/discount-sal-types/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    const isConfirmed = await confirm({
      ...
      description: "هل انت متأكد من حذف هذا الخصم المالي؟",
      icon: 'danger'
    });
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/discount-sal-types/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json'
