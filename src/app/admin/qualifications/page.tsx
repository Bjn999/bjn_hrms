...
export default function QualificationsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch('http://localhost:8000/api/admin/qualifications', {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = modalType === 'add' 
        ? 'http://localhost:8000/api/admin/qualifications' 
        : `http://localhost:8000/api/admin/qualifications/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`http://localhost:8000/api/admin/qualifications/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json'
