...
export default function RewardsDetailPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`${API}/rewards/${monthId}`, { headers: headers() });
      ...
      <div className="flex justify-between items-start mb-8 flex-wrap gap-4">
        <div>
          ...
          <div className="flex items-center gap-3">
            <Link
              href={`/portal/rewards/print/${monthId}`}
              target="_blank"
            ...
            {isMonthOpen && hasPermission('create_rewards') && (
              <button
        ...
        <div className="p-6 overflow-x-auto">
          ...
          {filteredRewards.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-slate-500 font-bold">لا يوجد مكافآت</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden relative z-10 animate-scale-up flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="text-lg font-black text-slate-800">{editingId ? 'تعديل المكافأة' : 'إضافة مكافأة جديدة'}</h3>
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">نوع المكافأة *</label>
                <select
                  value={form.additions_type_id}
                  onChange={e => setForm(f => ({ ...f, additions_type_id: e.target.value }))}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-700 font-bold focus:ring-2 focus:ring-indigo-500/20"
                >
            ...
            <div className="p-6 border-t border-slate-100 flex gap-3 justify-end">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-6 py-2.5 bg-gradient-to-r from-indigo-500 to-blue-600 text-white rounded-xl font-bold hover:shadow-lg hover:shadow-indigo-500/30 transition-all disabled:opacity-60"
              >
