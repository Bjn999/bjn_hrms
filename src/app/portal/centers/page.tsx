...
export default function CentersPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/centers`, {
        headers: {
  ...
  const fetchGovernorates = async (countryId: string) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/governorates-by-country/${countryId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
          'Accept-Language': language
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const url = modalType === 'add' ? `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/centers` : `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/centers/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/centers/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
          ...
          <table className="w-full text-sm text-right">
            ...
            <tbody>
              {data.length === 0 && !loading && (
                <tr>
                  <td colSpan={7} className="px-6 py-8 text-center text-slate-500 font-bold">{t('no_data')}</td>
                </tr>
              )}
              {data.map((item: CenterItem) => (
                <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 flex justify-center gap-2">
                    ...
                    {hasPermission('delete_centers') && !(item.counterUsed && item.counterUsed > 0) && (
