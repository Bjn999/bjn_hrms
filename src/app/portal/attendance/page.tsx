...
export default function AttendanceMonthsPage() {
  ...
  return (
    <div className="animate-fade-in-up pb-10">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h2 className="text-3xl font-black text-slate-800 tracking-tight">{t('fingerprint_menu')}</h2>
          <p className="text-slate-500 mt-1 text-sm">استعراض وإدارة سجلات الحضور والبصمة للموظفين مقسمة حسب الأشهر المالية</p>
        </div>
      </div>
        ...
        <div className="flex items-end gap-4 flex-wrap">
          <div>
            ...
            <select
              ...
              className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 outline-none text-slate-700 font-bold transition-all"
            >
              ...
              {data.map((item: FinanceMonth) => {
                return (
                  <tr key={item.id} className={`border-b border-slate-50 transition-colors ${item.is_open == 2 ? 'bg-rose-50/30' : 'hover:bg-slate-50/50'}`}>
                    <td className="px-5 py-4 font-bold text-slate-800">{item.month?.name_en}</td>
                      ...
                      <div className="flex items-center justify-center gap-3">
                        {item.is_open == 1 ? (
                          <>
                            ...
                            <Link
                              href={`/portal/attendance/${item.id}`}
