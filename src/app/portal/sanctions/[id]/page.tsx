...
const SANCTION_TYPES: Record<number, { label: string; color: string }> = {
  ...
  2: { label: 'جزاء بصمة', color: 'bg-orange-100 text-orange-700' },
  3: { label: 'جزاء تحقيق', color: 'bg-purple-100 text-purple-700' },
};
...
export default function SanctionsDetailPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`${API}/sanctions/${monthId}`, { headers: headers() });
      const result = await res.json();
      if (result.status) {
        setFinanceMonth(result.finance_month);
        setSanctions(result.sanctions || []);
        setEmployees(result.employees || []);
        // Month open check: finance year is open AND this month is_open
        const fm = result.finance_month;
        setIsMonthOpen(fm?.is_open == 1);
      } else {
        showToast(result.message, 'error');
        router.push('/portal/sanctions');
      }
    } catch {
  ...
  const handleSave = async () => {
    if (!form.employee_code || !form.sanctions_type || !form.value || !form.day_price) {
    ...
    if (!editingId) {
      ...
      if (exists) {
        const ok = await confirm({ title: t('warning'), description: t('confirm_duplicate_sanction'), icon: 'warning' });
        if (!ok) return;
      }
    }
    ...
    setSaving(true);
    try {
      const url = editingId ? `${API}/sanctions/${editingId}` : `${API}/sanctions`;
      ...
      const result = await res.json();
      if (result.status) {
        showToast(editingId ? t('sanction_updated') : t('sanction_added'), 'success');
        setShowModal(false);
        fetchData();
      } else {
        showToast(result.message, 'error');
  ...
  const handleDelete = async (s: SanctionItem) => {
    ...
    const ok = await confirm({ title: t('confirm_delete'), description: t('confirm_delete_sanction'), icon: 'danger' });
    if (!ok) return;
    try {
      const res = await fetch(`${API}/sanctions/${s.id}`, { method: 'DELETE', headers: headers() });
      const result = await res.json();
      if (result.status) { showToast(t('sanction_deleted'), 'success'); fetchData(); }
  ...
  const filteredSanctions = sanctions.filter((s: SanctionItem) => {
    ...
    const empCode = s.employee_code?.toString() || '';
    const typeLabel = SANCTION_TYPES[s.sanctions_type]?.label?.toLowerCase() || '';
      ...
      <div className="flex justify-between items-start mb-8 flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <Link href="/portal/sanctions" className="p-2 rounded-xl hover:bg-slate-100 transition-colors text-slate-500">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
            </Link>
            <h2 className="text-2xl font-black text-slate-800">{t('sanctions_days_title')}</h2>
          </div>
          {financeMonth && (
        ...
        <div className="flex items-center gap-3">
          <Link
            href={`/portal/sanctions/print/${monthId}`}
            target="_blank"
          ...
          {isMonthOpen && hasPermission('create_sanctions') && (
            <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-rose-50 flex items-center justify-center">
            <svg className="w-6 h-6 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" /></svg>
          </div>
          <div>
            <p className="text-xs text-slate-500 font-bold uppercase">{t('sanctions_list')}</p>
            <p className="text-2xl font-black text-slate-800">{sanctions.length}</p>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-slate-500 font-bold uppercase">{t('sanctions_days_count')}</p>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-slate-500 font-bold uppercase">{t('sanction_total')}</p>
          ...
          <table className="w-full text-sm text-right">
            <thead className="bg-slate-50 text-slate-700 uppercase border-b border-slate-100">
              <tr>
                ...
                <th className="px-5 py-4">{t('sanction_type')}</th>
                <th className="px-5 py-4">{t('sanctions_days_count')}</th>
                <th className="px-5 py-4">{t('sanction_total')}</th>
            ...
            <tbody>
              {filteredSanctions.map((s: SanctionItem) => {
                ...
                return (
                  <tr key={s.id} className={`border-b border-slate-50 transition-colors ${isArchived ? 'opacity-60 bg-slate-50/30' : 'hover:bg-slate-50/50'}`}>
                    <td className="px-5 py-4">
                      <div className="font-bold text-slate-800">
                        {s.employee ? `(${s.employee_code}) - ${s.employee.emp_name}` : s.employee_code}
                      </div>
                      {s.notes && (
                    ...
                    <td className="px-5 py-4 text-xs font-bold text-slate-500">
                      {s.updated_by && s.updated_by > 0 && s.updated_at ? (
                        <>
                          ...
                          <div className="text-emerald-600 mt-1">{s.updatedby?.name}</div>
                        </>
                      ) : (
                    ...
                    <td className="px-5 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {hasPermission('delete_sanctions') && (
                          <button
        ...
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden relative z-10 animate-scale-up flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="text-lg font-black text-slate-800">{editingId ? t('edit_sanction') : t('add_sanction')}</h3>
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">{t('sanction_type')} *</label>
                <select
                  value={form.sanctions_type}
                  onChange={e => setForm(f => ({ ...f, sanctions_type: e.target.value }))}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-700 font-bold focus:ring-2 focus:ring-emerald-500/20"
                >
                  <option value="">{t('select_option')}</option>
                  <option value="1">{t('sanction_type_days')}</option>
                  <option value="2">{t('sanction_type_fingerprint')}</option>
                  <option value="3">{t('sanction_type_investigation')}</option>
                </select>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">{t('sanctions_days_count')} *</label>
                  <input
              ...
              {form.value && form.day_price && (
                <div className="bg-rose-50 rounded-xl px-4 py-3">
                  <span className="text-xs text-rose-600 font-bold">{t('sanction_total')}: </span>
              ...
              <div>
                ...
                <textarea
                  ...
                  maxLength={100}
                  rows={2}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-700 font-bold focus:ring-2 focus:ring-emerald-500/20 resize-none"
                  placeholder={t('notes')}
                />
              </div>
            </div>
            <div className="p-6 border-t border-slate-100 flex gap-3 justify-end">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-6 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl font-bold hover:shadow-lg hover:shadow-emerald-500/30 transition-all disabled:opacity-60"
              >
