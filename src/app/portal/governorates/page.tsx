...
import { Governorate, Country } from '@/types';
import { usePermissions } from '@/hooks/usePermissions';

interface GovernorateItem extends Governorate {
  counterUsed?: number;
  country?: Country;
}
...
export default function GovernoratesPage() {
  ...
  const [data, setData] = useState<GovernorateItem[]>([]);
  const [countries, setCountries] = useState<Country[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [modalType, setModalType] = useState('add');
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editingItem, setEditingItem] = useState<GovernorateItem | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/governorates`, {
        headers: { 
  ...
  const fetchCountries = async () => {
    try {
      ...
      const result = await res.json();
      if (result.status) setCountries(result.data);
    } catch {
      showToast(t('fetch_failed'), 'error');
    }
  };
  ...
  const handleOpenModal = (type: string, item: GovernorateItem | null = null) => {
    setModalType(type);
    if (type === 'edit' && item) {
      ...
      setEditingId(null);
      setEditingItem(null);
      setFormData({ name: '', country_id: '', active: '1' });
    }
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const url = modalType === 'add' ? `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/governorates` : `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/governorates/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    const isConfirmed = await confirm({
      ...
      description: 'هل أنت متأكد من رغبتك في حذف هذه المحافظة؟',
      icon: 'danger'
    });
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/governorates/${id}`, {
        method: 'DELETE',
        headers: { 
          'Authorization': `Bearer ${token}`, 
          'Accept': 'application/json',
  ...
  return (
    <div className="animate-fade-in-up pb-10">
      <div className="flex justify-between items-end mb-8">
        <h2 className="text-3xl font-black text-slate-800 tracking-tight">{t('governorates')}</h2>
        {hasPermission('create_governorates') && (
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden relative">
        ...
        <table className="w-full text-sm text-right">
          <thead className="bg-slate-50 text-slate-700 uppercase border-b border-slate-100">
            <tr>
              <th className="px-6 py-4">{t('governorate_name')}</th>
          ...
          <tbody>
            {data.length === 0 && !loading && (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-slate-500 font-bold">{t('no_data')}</td>
              </tr>
            )}
            {data.map((item: GovernorateItem) => (
              <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                <td className="px-6 py-4 font-bold text-slate-900">{item.name}</td>
                <td className="px-6 py-4 text-slate-900 font-medium">{item.country?.name || '---'}</td>
                ...
                <td className="px-6 py-4 flex justify-center gap-2">
                  {hasPermission('edit_governorates') && (
                    <button onClick={() => handleOpenModal('edit', item)} className="p-2 text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title={t('edit')}>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                    </button>
                  )}
                  {hasPermission('delete_governorates') && !(item.counterUsed && item.counterUsed > 0) && (
                    <button onClick={() => handleDelete(item.id)} className="p-2 text-rose-500 hover:bg-rose-50 rounded-lg transition-colors" title={t('delete')}>
      ...
      {mounted && showModal && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8 animate-zoom-in relative overflow-hidden">
            ...
            <h3 className="text-2xl font-black text-slate-800 mb-6">{modalType === 'add' ? t('add_governorate') : t('edit_governorate')}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-base font-black text-slate-950 mb-2">{t('governorate_name')}</label>
              ...
              <div>
                <label className="block text-base font-black text-slate-950 mb-2">{t('country')}</label>
                <select name="country_id" value={formData.country_id} onChange={handleChange} required className="w-full px-4 py-3 bg-white border-2 border-slate-300 rounded-xl text-slate-950 font-black shadow-sm focus:ring-2 focus:ring-indigo-500/50 outline-none transition-all">
