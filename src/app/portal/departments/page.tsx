...
export default function DepartmentsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/departments`, {
        headers: {
  ...
  const handleOpenModal = (type: string, item: Department | null = null) => {
    setModalType(type);
    if (type === 'edit' && item) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = modalType === 'add' 
        ...
        : `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/departments/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/departments/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
    ...
    <>
      <div className="animate-fade-in-up pb-10">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h2 className="text-3xl font-black text-slate-800 tracking-tight">{t('departments')}</h2>
          </div>
          ...
          <div className="p-6 overflow-x-auto">
            <table className="w-full text-sm text-right">
              <thead className="bg-slate-50 text-slate-700 uppercase border-b border-slate-100">
                <tr>
                  <th className="px-6 py-4">{t('department_name')}</th>
                  <th className="px-6 py-4">{t('department_phone')}</th>
                  ...
                  <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {hasPermission('delete_departments') && !(item.counterUsed && item.counterUsed > 0) && (
                          <button 
      ...
      {mounted && showModal && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-6 relative z-10 animate-scale-up">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-slate-950">{modalType === 'edit' ? t('edit_department') : t('add_department')}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-base font-black text-slate-950 mb-2">{t('department_name')}</label>
              ...
              <div>
                <label className="block text-base font-black text-slate-950 mb-2">{t('department_phone')}</label>
