...
export default function LoansDetailPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`${API}/loans/${monthId}`, { headers: headers() });
      ...
      <div className="flex justify-between items-start mb-8 flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <Link href="/portal/loans" className="p-2 rounded-xl hover:bg-slate-100 transition-colors text-slate-500">
          ...
          {financeMonth && (
            <div className="flex items-center gap-3 flex-wrap mr-10">
              <span className="px-3 py-1 bg-sky-50 text-sky-700 rounded-lg text-sm font-bold">
        ...
        <div className="flex items-center gap-3">
          <Link
            href={`/portal/loans/print/${monthId}`}
            target="_blank"
        ...
        <div className="p-6 overflow-x-auto">
          ...
          {filteredLoans.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-slate-500 font-bold">{t('no_loans_found')}</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden relative z-10 animate-scale-up flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="text-lg font-black text-slate-800">{editingId ? 'تعديل سلفة' : 'إضافة سلفة'}</h3>
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">قيمة السلفة *</label>
                <input
              ...
              <div>
                ...
                <textarea
                  ...
                  maxLength={100}
                  rows={2}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-700 font-bold focus:ring-2 focus:ring-sky-500/20 resize-none"
                  placeholder={t('notes')}
                />
              </div>
            </div>
            <div className="p-6 border-t border-slate-100 flex gap-3 justify-end">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-6 py-2.5 bg-gradient-to-r from-sky-500 to-blue-500 text-white rounded-xl font-bold hover:shadow-lg hover:shadow-sky-500/30 transition-all disabled:opacity-60"
              >
