...
interface RewardTypeItem extends RewardType {
  counterUsed?: number;
  added?: User;
  updatedby?: User;
}

export default function RewardSalTypesPage() {
  ...
  const [data, setData] = useState<RewardTypeItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [modalType, setModalType] = useState('add');
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editingItem, setEditingItem] = useState<RewardTypeItem | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/reward-sal-types`, {
        headers: {
  ...
  const handleOpenModal = (type: string, item: RewardTypeItem | null = null) => {
    setModalType(type);
    if (type === 'edit' && item) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = modalType === 'add' 
        ? `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/reward-sal-types` 
        : `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/reward-sal-types/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    const isConfirmed = await confirm({
      ...
      description: 'هل أنت متأكد من حذف نوع المكافأة هذا؟',
      icon: 'danger'
    });
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/reward-sal-types/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
    ...
    <>
      <div className="animate-fade-in-up pb-10">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h2 className="text-3xl font-black text-slate-800 tracking-tight">{t('rewards_types')}</h2>
          </div>
          {hasPermission('create_reward_types') && (
            <button 
                  ...
                  <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        {hasPermission('edit_reward_types') && (
                        ...
                        {hasPermission('delete_reward_types') && !(item.counterUsed && item.counterUsed > 0) && (
      ...
      {mounted && showModal && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8 animate-zoom-in relative overflow-hidden">
            ...
            <h3 className="text-2xl font-black text-slate-800 mb-6">{modalType === 'add' ? t('add') : t('edit')}</h3>
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-base font-black text-slate-950 mb-2">{t('type')}</label>
