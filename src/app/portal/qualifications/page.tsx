...
import { Qualification, User } from '@/types';
import { usePermissions } from '@/hooks/usePermissions';

interface QualificationItem extends Qualification {
  counterUsed?: number;
  added?: User;
  updatedby?: User;
}
...
export default function QualificationsPage() {
  ...
  const [data, setData] = useState<QualificationItem[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/qualifications`, {
        headers: {
  ...
  const handleOpenModal = (type: string, item: QualificationItem | null = null) => {
    setModalType(type);
    if (type === 'edit' && item) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = modalType === 'add' 
        ? `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/qualifications` 
        : `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/qualifications/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/qualifications/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
    ...
    <>
      <div className="animate-fade-in-up pb-10">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h2 className="text-3xl font-black text-slate-800 tracking-tight">{t('qualifications')}</h2>
          </div>
          {hasPermission('create_qualifications') && (
            <button 
          ...
          <div className="p-6 overflow-x-auto">
            <table className="w-full text-sm text-right">
              <thead className="bg-slate-50 text-slate-700 uppercase border-b border-slate-100">
                <tr>
                  <th className="px-6 py-4">{t('qualification_name')}</th>
              ...
              <tbody>
                ...
                {data.map((item: QualificationItem) => (
                  <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        {hasPermission('edit_qualifications') && (
                          <button 
                        ...
                        {hasPermission('delete_qualifications') && !(item.counterUsed && item.counterUsed > 0) && (
                          <button 
      ...
      {mounted && showModal && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8 animate-zoom-in relative overflow-hidden">
            ...
            <h3 className="text-2xl font-black text-slate-800 mb-6">{modalType === 'add' ? t('add_qualification') : t('edit_qualification')}</h3>
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-base font-black text-slate-950 mb-2">{t('qualification_name')}</label>
