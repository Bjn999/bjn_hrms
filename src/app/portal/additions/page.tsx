...
interface AdditionsMonth extends FinanceMonth {
...
export default function AdditionsIndexPage() {
  const { t, language } = useLanguage();
  const { showToast } = useToast();

  const [data, setData] = useState<AdditionsMonth[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/additions${params}`, {
  ...
  const getMonthLabel = (item: AdditionsMonth) => {
  ...
  return (
    <div className="animate-fade-in-up pb-10">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h2 className="text-3xl font-black text-slate-800 tracking-tight">إضافي الأيام</h2>
          <p className="text-slate-500 mt-1 text-sm">إدارة السجلات الخاصة بإضافي أيام الموظفين مقسمة حسب الأشهر المالية</p>
        </div>
      </div>
