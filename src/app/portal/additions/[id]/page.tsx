...
export default function AdditionsDetailPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`${API}/additions/${monthId}`, { headers: headers() });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const url = editingId ? `${API}/additions/${editingId}` : `${API}/additions`;
      ...
      <div className="flex justify-between items-start mb-8 flex-wrap gap-4">
        <div>
          ...
          <div className="flex items-center gap-3">
            <Link
              href={`/portal/additions/print/${monthId}`}
              target="_blank"
            ...
            {isMonthOpen && hasPermission('create_additions') && (
              <button
        ...
        <div className="p-6 overflow-x-auto">
                  ...
                  <tr key={a.id} className={`border-b border-slate-50 transition-colors ${isArchived ? 'opacity-60 bg-slate-50/30' : 'hover:bg-slate-50/50'}`}>
                    ...
                    <td className="px-5 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {hasPermission('delete_additions') && (
                          <button
          ...
          {filteredAdditions.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-slate-500 font-bold">لا يوجد إضافي أيام</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden relative z-10 animate-scale-up flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="text-lg font-black text-slate-800">{editingId ? 'تعديل إضافي الأيام' : 'إضافة أيام إضافية'}</h3>
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">عدد الأيام *</label>
                  <input
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-700 font-bold focus:ring-2 focus:ring-emerald-500/20 cursor-not-allowed"
                    placeholder="0.00"
                  />
                </div>
              </div>
              {form.value && form.day_price && (
                <div className="bg-emerald-50 rounded-xl px-4 py-3 border border-emerald-100">
                  <span className="text-xs text-emerald-600 font-bold">الإجمالي: </span>
                  <span className="text-lg font-black text-emerald-700">{(parseFloat(form.value) * parseFloat(form.day_price)).toFixed(2)}</span>
                </div>
              )}
              <div>
