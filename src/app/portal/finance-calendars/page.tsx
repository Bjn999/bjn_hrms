...
export default function FinanceCalendarsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/finance-calendars`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = modalType === 'add'
        ? `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/finance-calendars`
        : `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/finance-calendars/${editingId}`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/finance-calendars/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
  ...
  const handleOpenYear = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/finance-calendars/${id}/open`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
  ...
  const handleShowMonths = async (id: number) => {
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/finance-calendars/${id}/months`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
          'Accept-Language': language
    ...
    <>
      <div className="animate-fade-in-up pb-10">
        <div className="flex justify-between items-end mb-8">
          ...
          {hasPermission('create_finance_calendars') && (
            <button
                  ...
                  <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${item.is_open === 1 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                        {item.is_open === 1 ? t('open') : t('closed')}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 flex items-center justify-center gap-2">
                      ...
                      {item.is_open === 0 && (
                        <>
                          {hasPermission('edit_finance_calendars') && (
                          ...
                          {hasPermission('delete_finance_calendars') && (
                          ...
                          {!hasOpen && hasPermission('edit_finance_calendars') && (
