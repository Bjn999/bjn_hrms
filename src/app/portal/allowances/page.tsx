...
interface AllowancesMonth extends FinanceMonth {
...
export default function AllowancesIndexPage() {
  const { t, language } = useLanguage();
  const { showToast } = useToast();

  const [data, setData] = useState<AllowancesMonth[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/employee-allowances${params}`, {
  ...
  const getMonthLabel = (item: AllowancesMonth) => {
  ...
  return (
    <div className="animate-fade-in-up pb-10">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h2 className="text-3xl font-black text-slate-800 tracking-tight">البدلات المتغيرة</h2>
          <p className="text-slate-500 mt-1 text-sm">إدارة السجلات الخاصة بالبدلات المتغيرة مقسمة حسب الأشهر المالية</p>
        </div>
      </div>
                      ...
                      <div className="flex items-center justify-center gap-3">
                        {item.is_open == 1 ? (
                          <>
                            ...
                            <Link
                              href={`/portal/allowances/${item.id}`}
