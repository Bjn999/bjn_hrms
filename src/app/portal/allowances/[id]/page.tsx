...
export default function EmployeeAllowancesDetailPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`${API}/employee-allowances/${monthId}`, { headers: headers() });
  ...
  const handleSave = async () => {
    if (!form.employee_code || !form.allowances_id || !form.total) {
    ...
    if (!editingId) {
      ...
      if (exists) {
        const ok = await confirm({ title: t('warning'), description: t('confirm_duplicate_allowance'), icon: 'warning' });
        if (!ok) return;
      }
    }
    ...
    setSaving(true);
    try {
      const url = editingId ? `${API}/employee-allowances/${editingId}` : `${API}/employee-allowances`;
      ...
      const result = await res.json();
      if (result.status) {
        showToast(editingId ? t('allowance_updated') : t('allowance_added'), 'success');
        setShowModal(false);
        fetchData();
      } else {
        showToast(result.message, 'error');
  ...
  const handleDelete = async (a: AllowanceItem) => {
    ...
    const ok = await confirm({ title: t('confirm_delete'), description: t('confirm_delete_allowance'), icon: 'danger' });
    if (!ok) return;
    try {
      const res = await fetch(`${API}/employee-allowances/${a.id}`, { method: 'DELETE', headers: headers() });
      const result = await res.json();
      if (result.status) { showToast(t('allowance_deleted'), 'success'); fetchData(); }
  ...
  return (
    <div className="animate-fade-in-up pb-10">
      ...
      <div className="flex justify-between items-start mb-8 flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <Link href="/portal/employee-salaries" className="p-2 rounded-xl hover:bg-slate-100 transition-colors text-slate-500">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
            </Link>
            <h2 className="text-2xl font-black text-slate-800">{t('changable_allowances')}</h2>
          </div>
          {financeMonth && (
        ...
        <div className="flex items-center gap-3">
          <Link
            href={`/portal/allowances/print/${monthId}`}
            target="_blank"
          ...
          {isMonthOpen && hasPermission('create_allowances') && (
            <button
              onClick={openAddModal}
              className="bg-gradient-to-r from-purple-500 to-fuchsia-600 text-white px-6 py-2.5 rounded-xl hover:shadow-lg hover:shadow-purple-500/30 hover:-translate-y-0.5 transition-all duration-300 font-bold flex items-center gap-2"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-black text-slate-800">{allowances.length}</p>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden relative">
        <div className="absolute top-0 right-0 w-full h-1 bg-gradient-to-l from-purple-500 to-fuchsia-400"></div>
        <div className="p-6 overflow-x-auto">
          <table className="w-full text-sm text-right">
            <thead className="bg-slate-50 text-slate-700 uppercase border-b border-slate-100">
              <tr>
                ...
                <th className="px-5 py-4">{t('allowance_type')}</th>
            ...
            <tbody>
              {filteredAllowances.map((a: AllowanceItem) => {
                ...
                const typeName = types.find((t: AllowanceType) => t.id == a.allowances_id)?.name || t('not_specified');
                ...
                return (
                  <tr key={a.id} className={`border-b border-slate-50 transition-colors ${isArchived ? 'opacity-60 bg-slate-50/30' : 'hover:bg-slate-50/50'}`}>
                    <td className="px-5 py-4">
                      ...
                      {a.notes ? (
                        <div className="relative group inline-block mt-2">
                          <button className="flex items-center gap-1 text-xs font-bold text-purple-700 bg-purple-50 px-2 py-1 rounded-lg hover:bg-purple-100 transition-colors">
                    ...
                    <td className="px-5 py-4 font-bold text-emerald-600 text-base">{parseFloat(String(a.total || 0)).toFixed(2)}</td>
                    ...
                    <td className="px-5 py-4 text-xs font-bold text-slate-500">
                      {a.updated_by && a.updated_by > 0 && a.updated_at ? (
                        <>
                          ...
                          <div className="text-rose-600 mt-1">{a.updatedby?.name}</div>
                        </>
                      ) : '-'}
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {hasPermission('delete_allowances') && (
                          <button
          ...
          {filteredAllowances.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-slate-500 font-bold">لا توجد بدلات متغيرة مسجلة</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden relative z-10 animate-scale-up flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="text-lg font-black text-slate-800">{editingId ? 'تعديل البدل' : 'إضافة بدل جديد'}</h3>
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">نوع البدل *</label>
                <select
                  value={form.allowances_id}
                  onChange={e => setForm(f => ({ ...f, allowances_id: e.target.value }))}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-700 font-bold focus:ring-2 focus:ring-purple-500/20"
                >
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">المبلغ الإجمالي *</label>
                <input
              ...
              <div>
                ...
                <textarea
                  ...
                  maxLength={100}
                  rows={2}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-700 font-bold focus:ring-2 focus:ring-purple-500/20 resize-none"
                  placeholder={t('notes')}
                />
              </div>
            </div>
            <div className="p-6 border-t border-slate-100 flex gap-3 justify-end">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-6 py-2.5 bg-gradient-to-r from-purple-500 to-fuchsia-600 text-white rounded-xl font-bold hover:shadow-lg hover:shadow-purple-500/30 transition-all disabled:opacity-60"
              >
