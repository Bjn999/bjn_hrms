...
interface DiscountsMonth extends FinanceMonth {
...
export default function DiscountsPage() {
  ...
  const [data, setData] = useState<DiscountsMonth[]>([]);
  ...
  const getMonthLabel = (item: DiscountsMonth) => {
  ...
  return (
    <div className="animate-fade-in-up pb-10">
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          <h2 className="text-3xl font-black text-slate-800 tracking-tight">الخصومات المالية</h2>
          <p className="text-slate-500 mt-1 text-sm">إدارة وعرض الخصومات المالية للموظفين حسب الأشهر المالية</p>
        </div>
      </div>
        ...
        <div className="flex items-end gap-4 flex-wrap">
          <div>
            ...
            <select
              ...
              className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-rose-500/20 outline-none text-slate-700 font-bold transition-all"
            >
        ...
        <div className="p-6 overflow-x-auto">
          <table className="w-full text-sm text-right">
            <thead className="bg-slate-50 text-slate-700 uppercase border-b border-slate-100">
              <tr>
                <th className="px-5 py-4 font-bold uppercase text-xs tracking-wider">{t('finance_month')}</th>
