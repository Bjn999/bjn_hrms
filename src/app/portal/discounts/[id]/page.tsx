...
export default function DiscountsDetailPage() {
  ...
  const handleSave = async () => {
    ...
    if (!editingId) {
      ...
      if (exists) {
        const ok = await confirm({ title: t('warning'), description: t('confirm_duplicate_discount'), icon: 'warning' });
        if (!ok) return;
      }
    }
    ...
    try {
      ...
      const result = await res.json();
      if (result.status) {
        showToast(editingId ? t('discount_updated') : t('discount_added'), 'success');
        setShowModal(false);
        fetchData();
      } else {
        showToast(result.message, 'error');
  ...
  const handleDelete = async (a: Discount) => {
    ...
    const ok = await confirm({ title: t('confirm_delete'), description: t('confirm_delete_discount'), icon: 'danger' });
    if (!ok) return;
    try {
      const res = await fetch(`${API}/discounts/${a.id}`, { method: 'DELETE', headers: headers() });
      const result = await res.json();
      if (result.status) { showToast(t('discount_deleted'), 'success'); fetchData(); }
      ...
      <div className="flex justify-between items-start mb-8 flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <Link href="/portal/discounts" className="p-2 rounded-xl hover:bg-slate-100 transition-colors text-slate-500">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
            </Link>
            <h2 className="text-2xl font-black text-slate-800">{t('financial_discounts')}</h2>
          </div>
          {financeMonth && (
          ...
          <div className="flex items-center gap-3">
            <Link
              href={`/portal/discounts/print/${monthId}`}
              target="_blank"
            ...
            {isMonthOpen && hasPermission('create_discounts') && (
              <button
                  ...
                  <tr key={a.id} className={`border-b border-slate-50 transition-colors ${isArchived ? 'opacity-60 bg-slate-50/30' : 'hover:bg-slate-50/50'}`}>
                    ...
                    <td className="px-5 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {hasPermission('delete_discounts') && (
                          <button
