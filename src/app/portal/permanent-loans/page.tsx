...
export default function PermanentLoansPage() {
  ...
  const handleSave = async () => {
    ...
    if (!editingId) {
      const exists = loans.some((a: PermanentLoan) => String(a.employee_code) === String(form.employee_code) && a.is_archived == 0);
      if (exists) {
  ...
  return (
    <div className="animate-fade-in-up pb-10">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-3">
          <Link
            href={`/portal/permanent-loans/print`}
            target="_blank"
          ...
          {hasPermission('create_permanent_loans') && (
            <button
                  ...
                  <tr key={a.id} className={`border-b border-slate-50 transition-colors ${isArchived ? 'opacity-60 bg-slate-50/30' : 'hover:bg-slate-50/50'}`}>
                    ...
                    <td className="px-5 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        {canAct && hasPermission('disburse_permanent_loans') && (
                          <button
                        ...
                        {hasPermission('edit_permanent_loans') && (
                          <button
                        ...
                        {hasPermission('delete_permanent_loans') && (
                          <button
                    ...
                    {computedInstallments.map((inst: Installment) => {
                      return (
                        <tr key={inst.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-5 py-4 text-center">
                            {inst.status == 0 && inst.is_archived == 0 && activeLoan?.is_disbursed == 1 && inst.counterbefornotpaid == 0 && hasPermission('pay_permanent_loans') && (
                              <button
