  ...
  const fetchSettings = async () => {
    try {
      ...
      if (rolesResult.status) {
        const filtered = (rolesResult.data || []).filter(
          (r: { name: string }) => r.name !== 'company_admin' && r.name !== 'super_admin'
        );
  ...
  return (
    <div className="max-w-6xl mx-auto animate-fade-in pb-20">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-black text-slate-800 tracking-tight">{t('add_employee')}</h2>
          <p className="text-slate-500 font-bold">{t('employee_code')}: ---</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <select name="role_id" value={formData.role_id} onChange={handleChange} className={inputClass}>
              ...
              {rolesList
                .filter((item) => item.name !== 'company_admin' && item.name !== 'super_admin')
                .map((item) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <label className={labelClass}>{t('salary')} <span className="text-rose-500">*</span></label>
        ...
        <div className="mt-12 flex gap-4">
          ...
          >
            {saving ? t('saving') : t('add_employee')}
          </button>
          <button
            type="button"
            onClick={() => router.back()}
