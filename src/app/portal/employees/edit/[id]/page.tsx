...
export default function EditEmployeePage() {
  ...
  const [rolesList, setRolesList] = useState<{ id: number; name: string, display_name?: string }[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      const [settingsRes, rolesRes, empRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/employees/required-data`, { headers }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/roles`, { headers }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/employees/${id}`, { headers })
      ]);
      ...
      if (rolesResult.status) setRolesList(rolesResult.data);
      ...
      if (empResult.status) {
        ...
        } else if (data.user?.roles?.[0]?.id) {
          resolvedRoleId = String(data.user.roles[0].id);
        } else if (data.user?.role) {
          const matched = rolesResult.data?.find((r: { name: string; id: number }) => r.name === data.user?.role);
          if (matched) resolvedRoleId = String(matched.id);
        }

        const isCompanyAdmin = Boolean(
          data.is_company_admin ||
          data.user?.role === 'company_admin' ||
          data.user?.roles?.some((r: { name: string }) => r.name === 'company_admin')
        );
        setIsCompanyAdminUser(isCompanyAdmin);
        delete cleanData.is_company_admin;
    ...
    if (type === 'file' && files) {
      ...
      setFormData((prev: EmployeeFormData) => {
        ...
        if (name === 'emp_sal') {
          ...
          newData.day_price = (sal / 21).toFixed(1);
        }
        return newData;
      });
    }
  ...
  const fetchSalaryArchive = async () => {
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/employees/${id}/salary-archive`, {
    ...
    <div className="max-w-6xl mx-auto animate-fade-in pb-20">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <label className={labelClass}>{language === 'ar' ? 'الدور / الصلاحيات في النظام' : 'System Role'}</label>
            <select
              name="role_id"
              value={formData.role_id}
              onChange={handleChange}
              disabled={isCompanyAdminUser}
              className={`${inputClass} ${isCompanyAdminUser ? 'bg-slate-100 cursor-not-allowed opacity-80' : ''}`}
            >
              <option value="">{language === 'ar' ? '-- اختر الدور --' : '-- Select Role --'}</option>
              {rolesList
                .filter((item) => isCompanyAdminUser || (item.name !== 'company_admin' && item.name !== 'super_admin'))
                .map((item) => (
            ...
            {isCompanyAdminUser && (
              <p className="text-xs font-bold text-amber-600 mt-1.5 flex items-center gap-1">
                <svg className="w-4 h-4 inline-block flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                {language === 'ar' ? 'لا يمكن تعديل دور مدير الشركة' : 'Company Admin role cannot be modified'}
              </p>
            )}
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <input type="number" name="day_price" value={formData.day_price} readOnly className={`${inputClass} bg-slate-100 cursor-not-allowed`} />
          </div>
          <div>
