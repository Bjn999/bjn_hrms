...
interface EmployeeShowData extends Omit<Employee, 'job' | 'social_status' | 'driving_license_type' | 'branch' | 'department'> {
...
export default function ShowEmployeePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const settingsRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/employees/required-data`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
          'Accept-Language': language
  ...
  const handleSubmitAllowance = async (e: React.FormEvent) => {
    ...
    if (!modalAllowanceId || !modalValue) {
      showToast('يجب ملء جميع الحقول المطلوبة', 'warning');
      return;
    }
    ...
    try {
      ...
      const url = modalMode === 'add'
        ? `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/employees/${id}/fixed-allowances`
        : `${process.env.NEXT_PUBLIC_API_URL || ''}/admin/employees/${id}/fixed-allowances/${editingAllowanceId}`;
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          allowance_id: modalAllowanceId,
          value: modalValue
        })
      });
      const result = await res.json();
      if (result.status) {
        showToast(modalMode === 'add' ? t('save_success') : t('update_success'), 'success');
        setShowModal(false);
        fetchData();
      } else {
  ...
  const handleDeleteAllowance = async (allowanceId: number) => {
    const isConfirmed = await confirm({
      ...
      description: 'هل أنت متأكد من رغبتك في حذف هذا البدل الثابت؟',
      icon: 'danger'
    });
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/employees/${id}/fixed-allowances/${allowanceId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
  ...
  const handleAddFile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/employees/${id}/files`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
  ...
  const handleDeleteFile = async (fileId: number) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/admin/employees/files/${fileId}`, {
        method: 'DELETE',
    ...
    <div className="animate-fade-in-up pb-10">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-3">
          <button 
            onClick={() => router.push(`/portal/employees/edit/${id}`)}
