...
export default function GenericSettingsPage({ 
  ...
  const moduleKey = apiEndpoint.replace(/-/g, '_');
  const canCreate = hasPermission(createPermission || `create_${moduleKey}`);
  const canEdit = hasPermission(editPermission || `edit_${moduleKey}`);
  const canDelete = hasPermission(deletePermission || `delete_${moduleKey}`);
              ...
              {data.map((item: SettingItem) => (
                <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 flex justify-center gap-2">
                    {canEdit && (
                      <button onClick={() => handleOpenModal('edit', item)} className="text-amber-500 p-2 hover:bg-amber-50 rounded-lg transition-colors" title={t('edit')}>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                      </button>
                    )}
                    {canDelete && !(item.counterUsed && item.counterUsed > 0) && (
                      <button onClick={() => handleDelete(item.id)} className="text-rose-500 p-2 hover:bg-rose-50 rounded-lg transition-colors" title={t('delete')}>
